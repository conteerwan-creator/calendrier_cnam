...
(async () => {
  console.log(
    'Diagnostic identifiants (sans exposer leur contenu) — longueur identifiant :',
    USERNAME.length,
    '| dernier code caractère :',
    USERNAME.charCodeAt(USERNAME.length - 1),
    '| longueur mot de passe :',
    PASSWORD.length,
    '| dernier code caractère mdp :',
  ...
  await page.type('#identifiant', USERNAME, { delay: 60 });
  await page.type('#mdp', PASSWORD, { delay: 60 });
  await Promise.all([
  ...
  if (choiceVisible) {
    ...
    if (needsCredentials) {
      ...
      await popup.type('#identifiant', USERNAME, { delay: 60 });
      await popup.type('#mdp', PASSWORD, { delay: 60 });
  ...
  for (const ev of allEvents) {
    ...
    const cleanTitle = stripCourseCode(ev.title);
